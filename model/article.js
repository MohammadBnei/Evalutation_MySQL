...
module.exports = {
    ...
    async createArticle(article, categories) {
        ...
        if (categories && categories.length) await pool.query(sqlLib.buildCategoryLinkToArticle(query.insertId, categories));
    ...
    async updateArticle(article, categories) {
        ...
        if (categories && categories.length) categories.forEach((category) => pool.query(sqlLib.buildCreateQuery({
